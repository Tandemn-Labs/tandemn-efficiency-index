...
Current repo: tandemn-labs/tandemn-efficiency-index
...
- After every code change, provide a concise summary of what was generated or changed. Also list each new or materially changed core function and explain its purpose in plain language.
...
This repository implements Tandemn Efficiency Index (TEI). The current MVP is
the Python Kubernetes and Prometheus telemetry path described in `README.md` and
`OBSERVABILITY.md`. All TEI application code is Python; deployment manifests and tool
configuration use their native declarative formats.
